...
- **Relationship creation has guardrails.** Manual drag-to-connect (the Plan-1 `onConnect` handler) stays, alongside the AI's `add_relationship`/`update_relationship`/`delete_relationship` tool calls — both paths go through the same `addRelationship` mutation function, which now rejects two cases: a field relating to itself, and a duplicate relationship between the same two fields (checked in both directions). Self-referencing tables (two different fields on the same table, e.g. `employees.manager_id` → `employees.id`) remain valid. The canvas also gets a client-side `isValidConnection` check so an invalid drag never visually "snaps" in the first place, not just fails after a round-trip. Manually-created relationships default to `one-to-many` cardinality with no `ai_comment`; the AI can still refine cardinality/description on any relationship (manual or AI-created) via `update_relationship`.
...
- Manual deletion of a relationship from the UI — no delete-edge affordance exists in this pass (creation is manual+AI with guardrails per UI/Layout; deletion is currently AI-only via `delete_relationship`, simply because no manual delete-edge UI was ever built, in this pass or Plan 1).
